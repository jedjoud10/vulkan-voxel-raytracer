...
const NORMAL_INSET_STRENGTH: f32 = -0.1f32;

// enables the bits for a specific location
fn enable_bitmask_radius(position: vek::Vec3<i32>, mask: &mut u64) {
...
pub fn dda_bake_mask(
    ...
    let mut floored_pos = (ray_pos * 4.0).floor().as_::<i32>();
    let inv_dir = ray_dir.map(|x| x.abs().max(1e-5)).recip();
    let dir_sign = ray_dir.map(|x| x.signum());
    let mut side_dist = ((dir_sign * (floored_pos.as_::<f32>() - ray_pos) + dir_sign * 0.5 + 0.5) * inv_dir);
    let mut mask = 0u64;
    ...
    for i in 0..16 {
        ...
        enable_bitmask_radius(floored_pos, &mut mask);
...
// +------------+----------------------+----------------------+
// | face pair  | face-1 segment index | face-2 segment index |
// +------------+----------------------+----------------------+
...
pub fn bake_intersection(
    ...
    for x_1 in 0..4 {
        for y_1 in 0..4 {
            let face_1_segment_index = x_1 + y_1 * 4;
            assert!(face_1_segment_index < 16);

            let uv_1 = vek::Vec2::new(x_1, y_1).map(|i| i as f32 / 4.0 + 0.25f32 * 0.5f32);
            ...
            for x_2 in 0..4 {
                for y_2 in 0..4 {
                    let face_2_segment_index = x_2 + y_2 * 4;
                    assert!(face_2_segment_index < 16);

                    let uv_2 = vek::Vec2::new(x_2, y_2).map(|i| i as f32 / 4.0 + 0.25f32 * 0.5f32);
                    ...
                    let index = face_pair_index << 8 | face_1_segment_index << 4 | face_2_segment_index;
                    assert!(index < COUNT);
